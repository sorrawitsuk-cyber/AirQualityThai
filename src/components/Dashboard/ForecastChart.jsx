...
const CustomXAxisTick = ({ x, y, payload, chartData }) => {
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      <foreignObject x={-40} y={10} width={80} height={130}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', fontSize: '0.75rem', fontWeight: 'bold', fontFamily: 'Kanit', textAlign: 'center', lineHeight: 1.2 }}>
          <span style={{ color: '#fff', fontSize: '0.8rem' }}>{item.time}</span>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', marginTop: '4px' }}>
            <span style={{ fontSize: '1.4rem', filter: 'drop-shadow(0px 2px 3px rgba(0,0,0,0.4))' }}>{item.icon || (item.time.startsWith('18') || item.time.startsWith('19') || item.time.startsWith('20') || parseInt(item.time) < 6 ? '🌙' : '☀️')}</span>
            <span style={{ color: '#60a5fa', fontSize: '0.75rem', fontWeight: 'bold' }}>{item.rain}%</span>
          </div>
          <span style={{ color: '#f97316', fontSize: '0.7rem', marginTop: '4px' }}>🥵 {item.feelsLike}°</span>
          <span style={{ color: pmColor, marginTop: '3px', background: 'rgba(0,0,0,0.2)', padding: '1px 6px', borderRadius: '10px' }}>😷 {item.pm25}</span>
        </div>
      </foreignObject>
    </g>
  );
       ...
       >
         <div style={{ width: '1400px', height: '220px' }}>
           <ResponsiveContainer width="100%" height="100%">
             <AreaChart data={chartData} margin={{ top: 20, right: 15, left: 15, bottom: 95 }}>
               <defs>
