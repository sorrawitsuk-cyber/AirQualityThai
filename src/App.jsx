...
const getUvHealthAdvice = (val) => { return (isNaN(val)||val===null)?null:val>10?{text:"หลีกเลี่ยงการออกแดดเด็ดขาด ผิวหนังอาจไหม้",icon:"⛔"}:val>=8?{text:"อยู่ในที่ร่ม ทากันแดด SPF50+ ใส่เสื้อแขนยาว",icon:"☂️"}:val>=6?{text:"ทาครีมกันแดด สวมหมวก หรือกางร่ม",icon:"🧢"}:null; };
...
export default function App() {
  ...
  const fetchAirQuality = async (isBg = false) => {
    if (!isBg) setLoading(true);
    try {
      const res = await fetch(`/api-air/services/getNewAQI_JSON.php?_t=${new Date().getTime()}`, { cache: 'no-store' });
      const data = await res.json();
    ...
    } catch (err) { console.error("Fetch AQI error:", err); } 
  ...
  const fetchAdvancedTemperatures = async (stations) => {
    ...
    for (let i = 0; i < stations.length; i += 25) {
      const chunk = stations.slice(i, i + 25);
      ...
      } catch (err) { console.error("Temp batch error", err); }
    ...
    try {
      ...
      if (dW.daily && dW.daily.time) {
        for (let i=0; i<dW.daily.time.length; i++) {
          ...
          let avgPm = 0;
          if (dA.hourly && dA.hourly.pm2_5) { const hrs = dA.hourly.pm2_5.slice(i*24, (i*24)+24).filter(v=>v!==null); avgPm = hrs.length>0 ? Math.round(hrs.reduce((a,b)=>a+b,0)/hrs.length):0; }
          let item = {
            date: dObj.toLocaleDateString('th-TH',{day:'numeric',month:'short'}),
            dayName: ['อา.','จ.','อ.','พ.','พฤ.','ศ.','ส.'][dObj.getDay()],
            temp: dW.daily.temperature_2m_max[i]||0, heat: dW.daily.apparent_temperature_max[i]||0,
            rain: dW.daily.precipitation_sum[i]||0, wind: dW.daily.wind_speed_10m_max[i]||0,
            uv: dW.daily.uv_index_max?(dW.daily.uv_index_max[i]||0):0, pm25: avgPm
          };
          if (i<14) {
  ...
  const fetchAlertsData = async (lat, lon, locName) => {
    ...
    try {
      const [rW, rA] = await Promise.all([fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&hourly=temperature_2m,apparent_temperature,precipitation_probability,precipitation,uv_index&forecast_days=2&timezone=Asia%2FBangkok`), fetch(`https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${lat}&longitude=${lon}&hourly=pm2_5&forecast_days=2&timezone=Asia%2FBangkok`)]);
      const [dW, dA] = await Promise.all([rW.json(), rA.json()]);
      let alerts = []; const nIdx = new Date().getHours();
      let mRainP=0, mRainV=0, rTime='', mHeat=0, hTime='', mUv=0, uTime='', mPm=0, pTime='';
      if(dW.hourly && dW.hourly.time) {
        dW.hourly.time.slice(nIdx, nIdx+24).forEach((t, i) => {
          const idx = nIdx + i;
          if (dW.hourly.precipitation_probability[idx] > mRainP) { mRainP=dW.hourly.precipitation_probability[idx]; mRainV=dW.hourly.precipitation[idx]; rTime=t; }
          if (dW.hourly.apparent_temperature[idx] > mHeat) { mHeat=dW.hourly.apparent_temperature[idx]; hTime=t; }
          if (dW.hourly.uv_index[idx] > mUv) { mUv=dW.hourly.uv_index[idx]; uTime=t; }
          if (dA.hourly && dA.hourly.pm2_5 && dA.hourly.pm2_5[idx] > mPm) { mPm=dA.hourly.pm2_5[idx]; pTime=t; }
        });
      }
      const fmt = (iso) => `${new Date(iso).getHours()}:00 น.`;
      if (mRainP >= 40 || mRainV >= 0.5) alerts.push({ type:'rain', icon:'🌧️', color:'#3b82f6', title:'แจ้งเตือนฝนตก', desc:`โอกาสฝน ${mRainP}% ปริมาณ ${mRainV} mm. (ช่วงที่คาด: ${fmt(rTime)}) เตรียมพกร่ม` });
      if (mHeat >= 33) alerts.push({ type:'heat', icon:'🥵', color:'#ea580c', title:`อากาศร้อน${mHeat>=42?'อันตราย!':'เตือนภัย'}`, desc:`ดัชนีความร้อนจะแตะ ${mHeat.toFixed(1)}°C (${fmt(hTime)}) ${mHeat>=42?'งด':'ลด'}กิจกรรมกลางแจ้ง` });
      if (mPm >= 37.5) alerts.push({ type:'pm25', icon:'😷', color:'#f59e0b', title:'ระวังฝุ่น PM2.5', desc:`ฝุ่นอาจแตะ ${mPm.toFixed(1)} µg/m³ (${fmt(pTime)}) ${mPm>=75?'งดออกนอกอาคาร':'สวมหน้ากาก N95'}` });
      if (mUv >= 8) alerts.push({ type:'uv', icon:'☀️', color:'#a855f7', title:'UV สูงมาก', desc:`ดัชนี UV ถึงระดับ ${mUv} (${fmt(uTime)}) ระวังผิวไหม้ ทากันแดดและกางร่ม` });
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', width:'100vw', backgroundColor:themeBg, fontFamily:"'Kanit', sans-serif", overflowY:'auto' }}>
            ...
            <>
              ...
              <div className="hide-scrollbar" style={{ display: 'flex', alignItems: 'center', gap: '10px', backgroundColor: 'rgba(255,255,255,0.15)', padding: '6px 15px', borderRadius: '30px', overflowX: 'auto', whiteSpace: 'nowrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <select value={selectedProvince} onChange={(e) => { setSelectedProvince(e.target.value); setSelectedStationId(''); setActiveStation(null); setShowRadar(false); }} style={{ padding: '8px 12px', borderRadius: '20px', border: 'none', minWidth: '150px', outline: 'none', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  <select value={selectedStationId} onChange={(e) => { setSelectedStationId(e.target.value); const stat = allStations.find(s => s.stationID === e.target.value); if(stat) {setActiveStation(stat); setShowRadar(false);} }} style={{ padding: '8px 12px', borderRadius: '20px', border: 'none', minWidth: '220px', outline: 'none', cursor: 'pointer' }}>
      ...
      {currentPage === 'map' ? (
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
          <div style={{ display: 'flex', gap: '15px', flexDirection: window.innerWidth < 768 ? 'column' : 'row', padding: '15px', minHeight: window.innerWidth < 768 ? 'auto' : '60vh' }}>
            
            {/* MAP AREA */}
            <div style={{ flex: 7, borderRadius: '12px', overflow: 'hidden', position: 'relative', border: `1px solid ${borderColor}` }}>
              <div className="hide-scrollbar" style={{ position: 'absolute', top: '15px', right: '15px', zIndex: 500, background: darkMode ? 'rgba(30,41,59,0.9)' : 'rgba(255,255,255,0.9)', padding: '5px 10px', borderRadius: '30px', display: 'flex', gap: '8px', overflowX: 'auto', whiteSpace: 'nowrap' }}>
              ...
              <button onClick={handleFindNearest} disabled={locating} style={{ position: 'absolute', bottom: '25px', right: '15px', zIndex: 500, width: '44px', height: '44px', borderRadius: '50%', backgroundColor: cardBg, color: textColor, border: `1px solid ${borderColor}`, cursor: locating ? 'wait' : 'pointer' }}>{locating ? '⏳' : '🎯'}</button>

              {!showRadar && (
                <div style={{ position: 'absolute', bottom: '25px', left: '60px', zIndex: 500, background: darkMode ? 'rgba(30,41,59,0.95)' : 'rgba(255,255,255,0.95)', padding: '12px', borderRadius: '10px', border: `1px solid ${borderColor}` }}>
                    ...
                    <Marker key={station.stationID} position={[parseFloat(station.lat), parseFloat(station.long)]} icon={createCustomMarker(viewMode, mVal, tObj)} ref={el => markerRefs.current[station.stationID]=el} eventHandlers={{ click: () => setActiveStation(station) }}>
                      <Popup minWidth={260}>
                        <div style={{ textAlign: 'center', fontFamily: 'Kanit' }}>
                          ...
                          {tObj && (
                            <div style={{ padding: '12px', background: '#f1f5f9', borderRadius: '8px', fontSize: '0.85rem' }}>
                              <div style={{ marginBottom: '8px', fontSize: '1.1rem' }}>{getWeatherIcon(tObj.weatherCode).icon} {getWeatherIcon(tObj.weatherCode).text}</div>
            ...
            {/* SIDEBAR */}
            <div style={{ flex: 3, minWidth: window.innerWidth < 768 ? '100%' : '380px', maxWidth: window.innerWidth < 768 ? '100%' : '450px', backgroundColor: cardBg, borderRadius: '12px', display: 'flex', flexDirection: 'column', border: `1px solid ${borderColor}`, overflow: 'hidden' }}>
              <div style={{ padding: '15px', background: darkMode ? '#0f172a' : '#f0f9ff', borderBottom: `1px solid ${borderColor}`, display: 'flex', justifyContent: 'space-between' }}>
                ...
                <select value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} style={{ padding: '4px', borderRadius: '6px', backgroundColor: cardBg, color: textColor }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '15px' }}>
                {filteredStations.map((station) => {
                  ...
                  return (
                    <div key={station.stationID} ref={el=>cardRefs.current[station.stationID]=el} onClick={()=>setActiveStation(station)} style={{ display:'flex', flexDirection:'column', background:isActive?(darkMode?'#334155':'#f8fafc'):cardBg, border:isActive?'1px solid #3b82f6':`1px solid ${borderColor}`, borderLeft:`6px solid ${boxBg}`, borderRadius:'10px', padding:'15px', marginBottom:'15px', cursor:'pointer' }}>
                      <div style={{ display:'flex', justifyContent:'space-between' }}>
                        <div style={{ flex:1 }}>
                          <h4 style={{ margin:'0 0 2px 0', color:textColor, fontSize:'1rem' }}>{station.nameTH}</h4>
                          <p style={{ margin:0, color:'#3b82f6', fontSize:'0.8rem' }}>{extractProvince(station.areaTH)}</p>
                          <div style={{ marginTop:'10px', fontSize:'0.85rem', color:subTextColor }}>
                        ...
                        <div style={{ backgroundColor:boxBg, color:'#fff', width:'60px', height:'60px', borderRadius:'12px', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
                      ...
                      {hAdv && (isActive || ['🚨','🚑','⛔'].includes(hAdv.icon)) && (
                        <div style={{ marginTop:'12px', padding:'10px', background:darkMode?'#1e293b':'#f8fafc', borderRadius:'8px', display:'flex', gap:'8px' }}><span>{hAdv.icon}</span><span style={{fontSize:'0.8rem',color:textColor}}>{hAdv.text}</span></div>
                      )}
                          ...
                          {isPm25Mode ? (
                            <div style={{ height:'100px', display:'flex', alignItems:'flex-end', gap:'2px' }}>
                              {activeForecast ? activeForecast.map((d,i)=>{
                                ...
                                return <div key={i} style={{flex:1, height:'100%', display:'flex', flexDirection:'column', justifyContent:'flex-end', alignItems:'center'}}><span style={{fontSize:'8px', color:subTextColor}}>{d.val}</span><div style={{width:'100%', height:`${h}%`, backgroundColor:d.color}}></div><span style={{fontSize:'8px', color:subTextColor}}>{i%3===0?d.time:''}</span></div>
                            ...
                            <div style={{ height:'100px', display:'flex', alignItems:'flex-end', gap:'6px' }}>
                              {activeWeather ? (isHeatMode?activeWeather.heatForecast:isUvMode?activeWeather.uvForecast:isRainMode?activeWeather.rainForecast:isWindMode?activeWeather.windForecast:activeWeather.tempForecast).map((d,i)=>{
                                const m=Math.max(...(isHeatMode?activeWeather.heatForecast:isUvMode?activeWeather.uvForecast:isRainMode?activeWeather.rainForecast:isWindMode?activeWeather.windForecast:activeWeather.tempForecast).map(x=>x.val))+(isRainMode?10:5);
                                const h=Math.max((d.val/m)*100, 5);
                                return <div key={i} style={{flex:1, display:'flex', flexDirection:'column', justifyContent:'flex-end', alignItems:'center', height:'100%'}}><span style={{fontSize:'10px', color:d.colorInfo.color||subTextColor}}>{d.val}</span><div style={{width:'100%', height:`${h}%`, backgroundColor:d.colorInfo.bar, borderRadius:'2px 2px 0 0'}}></div><span style={{fontSize:'10px', color:i<=1?'#0ea5e9':subTextColor}}>{d.time}</span></div>
                              }) : <div style={{width:'100%',textAlign:'center',color:subTextColor,fontSize:'0.8rem'}}>กำลังโหลด...</div>}
                            </div>
                          )}
                        </div>
          ...
          <div style={{ padding: '15px' }}>
            <div style={{ backgroundColor: cardBg, borderRadius: '12px', padding: '20px', border: `1px solid ${borderColor}` }}>
              <h2 style={{ fontSize: '1.2rem', color: textColor, margin: '0 0 15px 0' }}>📊 ข้อมูลเชิงลึก: {dashTitle}</h2>
              {dashLoading ? <div style={{ textAlign:'center', color:subTextColor }}>⏳ กำลังโหลด...</div> : dashHistory.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: window.innerWidth < 1024 ? '1fr' : '1fr 1fr', gap: '20px' }}>
                  <div style={{ background: darkMode?'#0f172a':'#f8fafc', padding: '15px', borderRadius: '10px' }}>
                    <h3 style={{ fontSize: '1rem', color: textColor, textAlign: 'center' }}>⏳ ย้อนหลัง 14 วัน</h3>
                    <div style={{ height: '220px' }}>
                      <ResponsiveContainer>
                        {activeChart.type === 'bar' ? (
                          <BarChart data={dashHistory} margin={{ top:5, right:10, bottom:5, left:-20 }}><CartesianGrid strokeDasharray="3 3" stroke={borderColor} /><XAxis dataKey="date" stroke={subTextColor} fontSize={10} /><YAxis stroke={subTextColor} fontSize={10} /><Tooltip /><Bar dataKey={activeChart.key} fill={activeChart.color} />{activeChart.hasLY && <Bar dataKey={activeChart.keyLY} fill="#94a3b8" />}</BarChart>
                        ) : activeChart.type === 'area' ? (
                          <AreaChart data={dashHistory} margin={{ top:5, right:10, bottom:5, left:-20 }}><CartesianGrid strokeDasharray="3 3" stroke={borderColor} /><XAxis dataKey="date" stroke={subTextColor} fontSize={10} /><YAxis stroke={subTextColor} fontSize={10} /><Tooltip /><Area type="monotone" dataKey={activeChart.key} stroke={activeChart.color} fill={activeChart.color} fillOpacity={0.3} /></AreaChart>
                        ) : (
                          <LineChart data={dashHistory} margin={{ top:5, right:10, bottom:5, left:-20 }}><CartesianGrid strokeDasharray="3 3" stroke={borderColor} /><XAxis dataKey="date" stroke={subTextColor} fontSize={10} /><YAxis stroke={subTextColor} fontSize={10} /><Tooltip /><Line type="monotone" dataKey={activeChart.key} stroke={activeChart.color} strokeWidth={2} />{activeChart.hasLY && <Line type="monotone" dataKey={activeChart.keyLY} stroke="#94a3b8" strokeDasharray="4 4" />}</LineChart>
                        )}
                      </ResponsiveContainer>
                    </div>
                  </div>
                  <div style={{ background: darkMode?'#0f172a':'#f8fafc', padding: '15px', borderRadius: '10px' }}>
                    <h3 style={{ fontSize: '1rem', color: textColor, textAlign: 'center' }}>🔮 พยากรณ์ล่วงหน้า 7 วัน</h3>
                    <div style={{ height: '220px' }}>
                      <ResponsiveContainer>
                        {activeChart.type === 'bar' ? (
                          <BarChart data={dashForecast} margin={{ top:5, right:10, bottom:5, left:-20 }}><CartesianGrid strokeDasharray="3 3" stroke={borderColor} /><XAxis dataKey="date" stroke={subTextColor} fontSize={10} /><YAxis stroke={subTextColor} fontSize={10} /><Tooltip /><Bar dataKey={activeChart.key} fill={activeChart.color} /></BarChart>
                        ) : (
                          <AreaChart data={dashForecast} margin={{ top:5, right:10, bottom:5, left:-20 }}><CartesianGrid strokeDasharray="3 3" stroke={borderColor} /><XAxis dataKey="date" stroke={subTextColor} fontSize={10} /><YAxis stroke={subTextColor} fontSize={10} /><Tooltip /><Area type="monotone" dataKey={activeChart.key} stroke={activeChart.color} fill={activeChart.color} fillOpacity={0.3} /></AreaChart>
                        )}
                      </ResponsiveContainer>
                    </div>
                  </div>
                </div>
              ) : <div style={{ textAlign:'center', color:subTextColor }}>ไม่มีข้อมูลสถิติ</div>}
            </div>
          </div>
        </div>
      ) : (
        ...
        <div style={{ flex: 1, padding: '20px', maxWidth: '800px', margin: '0 auto', width: '100%' }}>
          <div style={{ backgroundColor: cardBg, borderRadius: '16px', padding: '30px', border: `1px solid ${borderColor}`, textAlign: 'center' }}>
            <h2 style={{ fontSize: '1.8rem', color: textColor, marginBottom: '10px' }}>🔔 ศูนย์แจ้งเตือนภัยอัจฉริยะ</h2>
            <p style={{ color: subTextColor, marginBottom: '25px' }}>สรุปสถานการณ์รอบตัวใน 24 ชั่วโมงข้างหน้า</p>
            <button onClick={handleScanLocation} disabled={alertsLoading} style={{ backgroundColor: '#0ea5e9', color: '#fff', border: 'none', borderRadius: '30px', padding: '15px 30px', fontSize: '1.2rem', fontWeight: 'bold', cursor: alertsLoading?'wait':'pointer', width: window.innerWidth < 768 ? '100%' : 'auto' }}>
              {alertsLoading ? '⏳ กำลังประมวลผล...' : '📍 ตรวจสอบพิกัดปัจจุบัน'}
            </button>
            {alertsLocationName && !alertsLoading && <div style={{ marginTop: '20px', padding: '10px', backgroundColor: darkMode?'#0f172a':'#f0f9ff', borderRadius: '10px', color: '#0ea5e9', fontWeight: 'bold' }}>พื้นที่: {alertsLocationName}</div>}
            ...
            <div style={{ marginTop: '40px', display: 'flex', flexDirection: 'column', gap: '15px', textAlign: 'left' }}>
              {alertsLoading ? null : alertsData.length > 0 ? (
                alertsData.map((al, idx) => (
                  <div key={idx} style={{ display: 'flex', gap: '15px', backgroundColor: darkMode?'#1e293b':'#fff', padding: '20px', borderRadius: '12px', borderLeft: `6px solid ${al.color}` }}>
                    <div style={{ fontSize: '2.5rem' }}>{al.icon}</div>
                    <div><h4 style={{ margin: '0 0 5px 0', fontSize: '1.2rem', color: al.color }}>{al.title}</h4><p style={{ margin: 0, color: textColor }}>{al.desc}</p></div>
                  </div>
                ))
              ) : alertsLocationName ? (
                <div style={{ display: 'flex', gap: '15px', backgroundColor: darkMode?'#1e293b':'#f0fdf4', padding: '20px', borderRadius: '12px', borderLeft: `6px solid #22c55e` }}>
                  <div style={{ fontSize: '2.5rem' }}>🟢</div>
                  <div><h4 style={{ margin: '0 0 5px 0', fontSize: '1.2rem', color: '#16a34a' }}>สภาพอากาศปลอดภัย</h4><p style={{ margin: 0, color: textColor }}>ไม่มีการแจ้งเตือนสภาพอากาศรุนแรงในพื้นที่ของคุณ</p></div>
                </div>
              ) : null}
            </div>
          </div>
