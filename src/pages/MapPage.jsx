...
function MapUpdater({ lat, lon }) {
  const map = useMap();
  useEffect(() => {
    if (lat && lon) map.flyTo([lat, lon], 11, { animate: true });
  }, [lat, lon, map]);
  return null;
}
...
export default function MapPage() {
  ...
  if (loading) {
    return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%', background: darkMode ? '#0f172a' : '#f0f9ff', color: darkMode ? '#fff' : '#000' }}>กำลังโหลดแผนที่... ⏳</div>;
  }
  ...
  const modes = [
    ...
    { id: 'temp', label: 'อุณหภูมิ', icon: '🌡️', color: '#eab308' },
    { id: 'rain', label: 'โอกาสฝน', icon: '☔', color: '#3b82f6' },
    { id: 'humidity', label: 'ความชื้น', icon: '💧', color: '#10b981' },
    { id: 'wind', label: 'ความเร็วลม', icon: '🌬️', color: '#db2777' },
  ];
      ...
      <div style={{ flex: 1, position: 'relative', zIndex: 1 }}>
        <MapContainer center={[13.7563, 100.5018]} zoom={6} style={{ height: '100%', width: '100%' }} zoomControl={false}>
          ...
          {safeStations.map(st => {
            ...
            else if (activeMode === 'wind') { valToShow = tObj.windSpeed != null ? Math.round(tObj.windSpeed) : '-'; circleColor = getWindColor(tObj.windSpeed); }

            const isSelected = selectedStation?.stationID === st.stationID;

            return (
              <CircleMarker 
                key={st.stationID} center={[lat, lon]} 
              ...
              >
                <Tooltip direction="top" offset={[0, -10]} opacity={1} permanent={false}>
                  <div style={{ textAlign: 'center', fontWeight: 'bold', fontFamily: 'Kanit' }}>
                    ...
                    <div style={{ fontSize: '1.1rem' }}>{valToShow}</div>
                  </div>
                </Tooltip>
              </CircleMarker>
            );
          ...
          <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '5px' }} className="hide-scrollbar">
            {modes.map(mode => (
              <button 
                ...
                style={{ 
                  display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', borderRadius: '14px', border: 'none', flexShrink: 0,
                  background: activeMode === mode.id ? mode.color : innerCardBg, 
                  color: activeMode === mode.id ? '#fff' : subTextColor,
                  fontWeight: 'bold', fontSize: '0.85rem', cursor: 'pointer', transition: 'all 0.2s',
                  boxShadow: activeMode === mode.id ? `0 4px 10px ${mode.color}40` : 'none'
                }}>
      ...
      {isMobile && selectedStation && (
        <div style={{ position: 'absolute', bottom: isMobileCardOpen ? '320px' : '90px', left: '50%', transform: 'translateX(-50%)', zIndex: 1000, transition: 'bottom 0.3s cubic-bezier(0.4, 0, 0.2, 1)' }}>
          <button 
            ...
            style={{
              background: '#0f172a', color: '#fff', border: `2px solid rgba(255,255,255,0.2)`, borderRadius: '50px',
            ...
            {isMobileCardOpen ? (
               ...
               <><span style={{ fontSize: '1.2rem' }}>📊</span> ดูข้อมูลสถานีนี้</>
            )}
          </button>
        </div>
      )}
      ...
      {selectedStation && (
        <div style={{ 
          ...
          ...(isMobile 
            ...
            : { top: '20px', right: '20px', width: '340px', borderRadius: '24px', maxHeight: 'calc(100vh - 40px)', overflowY: 'auto' }),
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
            ...
            {!isMobile && (
              <button onClick={() => setSelectedStation(null)} style={{ background: innerCardBg, border: 'none', width: '30px', height: '30px', borderRadius: '50%', color: subTextColor, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✖</button>
            )}
          </div>
                ...
                {gridItems.map((item, idx) => (
                  <div key={idx} style={{ background: innerCardBg, padding: '12px', borderRadius: '16px', border: `1px solid ${borderColor}`, display: 'flex', flexDirection: 'column' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                      <span style={{ fontSize: '1.2rem', filter: 'drop-shadow(0 2px 2px rgba(0,0,0,0.1))' }}>{item.icon}</span>
                      <span style={{ fontSize: '0.75rem', color: subTextColor, fontWeight: 'bold' }}>{item.label}</span>
                    </div>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: item.color === '#94a3b8' ? textColor : item.color, lineHeight: 1 }}>
